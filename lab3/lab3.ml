...
the type of the 'partition' is ('a -> bool) -> 'a list -> 'a list * 'a list 
...
1. What is the most general (polymorphic) type for its first argument
   ...
   The function to be applied takes an argument of some generic type.
   Instead of calling it `'a`, we can call it `'arg` to give it some
   meaning. The output type for this function to be applied also has
   a general type can we can call it `'result`.
   So the type of the first argument is `'arg -> 'result`.

2. What is the most general type for its second argument (the argument
   to apply it to)?

   The second argument is the argument we apply that function to, so
   it should have the same type as the input type of that function,
   which is `'arg`. 

3. What is the type of its result?
   
   The type of its result should match the type of output of the 
   function that we are applying, which is of type `'result`. 

4. Given the above, what should the type of the function `apply` be?

   Therefore, the type for apply should be
