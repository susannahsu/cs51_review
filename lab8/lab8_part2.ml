...
"deserialized" to recreate the internal representation. We'll add to
the stack module extremely basic support for serialization.

   (You may notice that this provides an opportunity to break an
...
function (`serialize`) that converts values of that type into a string
representation. The SERIALIZE module type is an appropriate signature
for the argument of a functor that generates serializable stack
modules. *)
